...
export const generateArticleContent = async (topic: string, tone: Tone, includeTags: boolean): Promise<ArticleContent> => {
  const prompt = `
    Generate a detailed and comprehensive news article about the following topic: "${topic}".
    The entire article must be written in Turkish.
    The tone of the article should be ${tone}.
    The total length of the article's content must be between 4500 and 6500 characters.
    
    The output must include:
    1. A compelling, SEO-friendly title.
    2. Extensive main content, broken into an array of multiple paragraphs.
    3. A short, SEO-optimized "metaDescription" (150-160 characters).
    ${includeTags ? '4. An array of 3-5 relevant "tags" (as strings).' : ''}

    Return the result as a single, minified JSON object with no markdown formatting.
