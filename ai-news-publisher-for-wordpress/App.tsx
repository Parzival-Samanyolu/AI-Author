...
const App: React.FC = () => {
  ...
  const [publishStatus, setPublishStatus] = useState<WordPressPublicationStatus>(WordPressPublicationStatus.Draft);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (includeImage) {
        const imagePrompt = `A high-quality, realistic news-style photo representing: "${articleContent.title}"`;
  ...
  }, [topic, tone, includeImage, includeTags]);
  ...
  const handleRegenerateImage = useCallback(async () => {
    ...
    try {
      const imagePrompt = `A high-quality, realistic news-style photo representing: "${generatedArticle.title}"`;
      const newImageUrl = await generateArticleImage(imagePrompt);

      if (newImageUrl) {
        setGeneratedArticle(prev => prev ? { ...prev, imageUrl: newImageUrl } : null);
      } else {
